...
writer = SummaryWriter(log_dir='/home/pankaj/AML-Models/fusion_models/logs/attention')
...
class Activity_Detection(nn.Module):
    ...
    def forward(self, resnet_features, c3d_features, audio_features):
        ...
        a_f = torch.unsqueeze(audio_final,1)
        r_f = torch.unsqueeze(resnet_final,1)
        c_f = torch.unsqueeze(c3d_final,1)
        final_f = torch.cat([a_f, r_f, c_f], dim=1)

        score_a = self.attn(audio_final)
        score_r = self.attn(resnet_final)
        score_c = self.attn(c3d_final)
