...
writer = SummaryWriter(log_dir='/home/pankaj/AML-Models/proposed/logs/pool_addition')
...
class Activity_Detection(nn.Module):
    def __init__(self, resnet_size, c3d_size, audio_size, projected_size, hidden_size, num_classes):
        ...
        self.fc_out = nn.Linear(1143, num_classes)

        # self.pool_max = nn.MaxPool2d()
        self.pool_avg = nn.AvgPool2d((3,3), stride=(2,2))
    ...
    def forward(self, resnet_features, c3d_features, audio_features):
        ...
        add_fusion_grid = audio_complete + resnet_complete + c3d_complete

        add_fusion_grid = self.pool_avg(add_fusion_grid)

        # print(add_fusion_grid.size())
        batch_size = resnet_features.shape[0]

        fused_out = add_fusion_grid.view(batch_size, -1)

        # print(fused_out.shape)

        predictions = self.fc_out(fused_out)
