...
writer = SummaryWriter(log_dir='/home/pankaj/AML-Models/Supervised/logs/max_pool_attention')
...
class Activity_Detection(nn.Module):
    ...
    def forward(self, resnet_features, c3d_features, audio_features):
        ...
        for i in range(features_size):
            a_f = torch.unsqueeze(audio_complete[:,i,:],1)
            r_f = torch.unsqueeze(resnet_complete[:,i,:],1)
            c_f = torch.unsqueeze(c3d_complete[:,i,:],1)
            final_f = torch.cat([a_f, r_f, c_f], dim=1)

            score_a = self.attn(audio_complete[:,i,:])
            score_r = self.attn(resnet_complete[:,i,:])
            score_c = self.attn(c3d_complete[:,i,:])

            a_s = torch.unsqueeze(score_a, 1)
            r_s = torch.unsqueeze(score_r, 1)
            c_s = torch.unsqueeze(score_c, 1) 
            final_s = torch.cat([a_s, r_s, c_s], dim=1)
            final_s = torch.transpose(final_s, 1, 2)

            fused_out = torch.bmm(final_s, final_f)
            fused_out = torch.squeeze(fused_out)
